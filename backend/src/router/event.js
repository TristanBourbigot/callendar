                            ...
                            if(req.body.place){
                                if(req.body.category){
                                    if(req.body.allDay !== null){
                                        let event = await createEvent(req.user[0].email, req.body.title, req.body.description, req.body.start, req.body.end, req.body.place, req.body.category, req.body.allDay);
                                        res.status(200).json(event);
                                    }else{
...
eventRouter.delete('/deleteEvent/:eventId', asyncHandler(async (req,res) => {
    if(req.params.eventId){
        let event = await deleteEvent(req.params.eventId);
        res.status(200).json(event)
    }else{
